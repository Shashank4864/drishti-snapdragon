...
- **Both GUI frameworks we tried hit the same underlying NPU crash; verified, reliable inference currently requires direct script execution.** `pyarrow`, a hard dependency of modern Streamlit, has no official prebuilt wheel for Windows ARM64 (confirmed via an open, unresolved upstream Apache Arrow GitHub issue); we worked around the packaging conflict and got the Streamlit UI to load correctly. However, inference calls made through Streamlit consistently crashed with a native Hexagon NPU error (`ggml-hex: dspqueue_read failed`). We then rebuilt the interface as a **native Tkinter desktop app** — no browser, no local server, part of Python's standard library — expecting this simpler execution model to avoid the issue. It did not: the same NPU crash occurred on the first inference call through Tkinter as well, reproducibly, even after a full device reboot to rule out leftover driver state. By contrast, the identical AI pipeline (`Vision.py` → `Language.py` → `Speech.py`) run as a **plain Python script**, with no GUI framework involved, has completed successfully many times across three separate QDC sessions with no crashes. This strongly suggests the issue is specific to how GenieX's native NPU queue interacts with a GUI event loop / thread, rather than anything in our model or pipeline code. All verified benchmark numbers in this document come from script-based execution; the Tkinter interface exists in the repository and demonstrates the intended user experience, but is not yet confirmed stable for live inference and should be treated as a UI prototype pending further investigation.
...
- **Speech output language depends entirely on installed Windows system voices, not on our code.** Our test devices (both a personal laptop and the Snapdragon X2 Elite QDC instance) had only English voice packs installed at the OS level, so spoken audio currently defaults to English even when the underlying translated text is correctly in Hindi, Telugu, or Tamil. The `speech.py` module actively scans installed system voices at runtime and automatically selects a matching voice for the requested language when one is present — on any Windows machine with the relevant language pack installed (Settings → Time & Language → Language & Region → Add a language → include "Speech" as a feature), the same unmodified code would produce native-language audio output with no changes required.
